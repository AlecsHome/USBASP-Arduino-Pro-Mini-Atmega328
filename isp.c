...
static uchar last_saved_speed = 0xFF;  // ƒелаем переменную доступной дл€ всех функций в файле
...
// ”лучшенна€ верси€ с защитой от износа EEPROM
void ispSaveSpeedToEEPROM(uchar speed) {
    // —охран€ем только если скорость изменилась и она валидна
    if (speed != last_saved_speed && 
        speed >= USBASP_ISP_SCK_0_5 && 
        speed <= USBASP_ISP_SCK_3000) {
...
void ispLoadLastSpeed(void) {
    uint8_t s = eeprom_read_byte((uint8_t *)EEPROM_SPEED_ADDR);
    if (s == 0xFF) {                    // свежее стирание
        s = USBASP_ISP_SCK_375;         // безопасна€
        eeprom_update_byte((uint8_t *)EEPROM_SPEED_ADDR, s);
    }
    last_success_speed = (s >= USBASP_ISP_SCK_0_5 && s <= USBASP_ISP_SCK_3000)
                         ? s : USBASP_ISP_SCK_AUTO;
}

// ‘ункци€ сброса сохраненной скорости
void ispResetStoredSpeed(void) {
    eeprom_update_byte((uint8_t *)EEPROM_SPEED_ADDR, 0xFF);
    
    // —брасываем статическую переменную дл€ принудительной записи при следующем сохранении
    // Ёто нужно, чтобы обойти защиту от частой записи
